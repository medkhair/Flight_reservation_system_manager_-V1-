...
class Flight{
	...
	public static Comparator<Flight> dateComparator = new Comparator<Flight>() {

		public int compare(Flight f1, Flight f2) {
			LocalDateTime d1 = f1.DepartingTime;
		    LocalDateTime d2 = f2.DepartingTime;
...
class Reservation{
	...
	void setReservation(ArrayList<String> cities, ArrayList<Flight> flights){
		...
		LocalDateTime check = LocalDateTime.of(y,m,d,23,59);
...
class FlightApp{
	public static void main(String[] args) {
		...
		cities.add("Dubai");
		cities.add("New York");
		Flight fl = new Flight("RyanAir","152478","Boing500",15000,0,20,DepartMadridMarrakech,"Madrid",ArriveMadridMarrakech,"Marrakech");
		Collections.sort(flights, Flight.dateComparator);
		do{
			System.out.println("-------------------------------------");
	...
	static void allAvailableFlights(ArrayList<Flight> flights){
		...
		do{
            ...
            for (i=0;i<flights.size();i++) {
            	if (flights.get(i).DepartingTime.isAfter(today) == false) {
            		flights.remove(i);
            	}
